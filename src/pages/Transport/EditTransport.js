...
const EditTransport = ({setIsEdit, detail}) => {
  const token = Cookies.get('jwtToken');

  const formList = {
    route_number: detail?.route_number,
    ...
    helper_name: detail?.helper_name,
    helper_phone: detail?.helper_phone,
    // live_tracking: detail?.live_tracking,
    transport_id: detail?.transport_id,
  };

  const [formData, setFormData] = useState(formList);
  const [errors, setErrors] = useState({});
  const [selectLiveTracker, setSelectLiveTracker] = useState({value: detail?.live_tracking, label: detail?.live_tracking});
