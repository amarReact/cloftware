...
const TableMUI = ({ data = [], columns = [], actions = [], onSelectionChange, pageList = 10 }) => {
            ...
            {rows.map(row => (
              <TableRow key={row.id}>
               {onSelectionChange && <TableCell>
                ...
                {columns.map(column => (
                  <TableCell hide={column?.hide && "none"} width={column?.width && column.width} attr-data={row[column.id]} key={column.id}>
